...
import { ChevronLeft, PanelLeftIcon, SidebarOpen } from "lucide-react";
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      aria-label="Åpne/lukk sidemenyen"
      tabIndex={-1}
      onClick={toggleSidebar}
      title="Åpne/lukk sidemenyen"
      className={cn(
        "group/rail",
        "hover:after:bg-accent absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[1px] sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
      ...
      {...props}
    >
      <SidebarOpen className="hidden [[data-side=left][data-state=expanded]_&]:rotate-180 group-hover/rail:block absolute top-1/2 -translate-y-1/2 left-5" />
    </button>
  );
}
