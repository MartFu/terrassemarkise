    ...
    <div className="flex-1">
      ...
      <ErrorBoundary context="ressurser">
        <div className="flex flex-col">
          ...
          <div className="flex justify-center py-8 border-y my-20">
            --- SECTION ROW 1 BELOW ---
          </div>
          ...
          <SectionRow1
            ...
            countLabel="artikler"
          >
            {/* <ArticlePreviewList items={artikler.items} /> */}
            {mockItems.map((item, idx) => (
              <Link
                key={`article-items-${item.slug}-${idx}`}
                href={`/ressurser/artikler/${item.slug}`}
              >
                <div
                  key={item.slug}
                  className="h-full aspect-video transition-colors bg-card/50 hover:bg-card/30 border border-border/40 p-4"
                >
                  <Stack preset="card">
                    <h3 className="text-foreground/70 text-base font-semibold leading-snug tracking-tight">
                      Tool Item {idx + 1}
          ...
          <SectionRow1
            ...
            countLabel="videoer"
          >
            {/* <VideoPreviewList items={videoer.items} /> */}
            {mockItems.map((item, idx) => (
              <Link
                key={`article-items-${item.slug}-${idx}`}
                href={`/ressurser/artikler/${item.slug}`}
          ...
          >
            ...
            {mockItems.map((item, idx) => (
              ...
              >
                <div
                  key={item.slug}
                  className="h-full w-80 aspect-video transition-colors bg-card/50 hover:bg-card/30 border border-border/40 p-4"
                >
                  <Stack preset="card">
                    <h3 className="text-foreground/70 text-base font-semibold leading-snug tracking-tight">
                      Article Item {idx + 1}
